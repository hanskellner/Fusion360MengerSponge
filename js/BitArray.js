 ...
 * or initialises the BitArray with the given hex representation
 *
 * Usage:
 *    var a = new BitArray(32);
 *    a.set(0, true);
 *    a.set(31, true);
 *    console.log( a.toString() ); // "10000000000000000000000000000001"
 *    console.log( a.get(1) ); // false
 *    console.log( a.get(31) ); // true
 *
 * @param {Number} size - The given size in bits
 * @param {Boolean} hex - Use the hex string as value
 * @constructor
 */
function BitArray(size, hex) {
    this.values = [];
 ...
 * @param {Number} index - The position to set at
 * @param {Boolean} value
 */
BitArray.prototype.set = function (index, value) {
	var i = Math.floor(index / 32);

	// Since "undefined | 1 << index" is equivalent to "0 | 1 << index"
    // we do not need to initialise the array explicitly here.
	if (value) {
 ...
 * If the bit is on, it is turned off.
 * Likewise, if the bit is off it is turned on.
 *
 * @param {Number} index
 */
...
BitArray.prototype.copy = function () {
	...
	cp.values = Array.prototype.concat(this.values);
	return cp;
};
 ...
 *
 * @param {Array} x - Another BitArray to compare with
 */
...
BitArray.prototype.count = function () {
	...
	// If we remove the toggle method
    // we could efficiently cache the number of bits
